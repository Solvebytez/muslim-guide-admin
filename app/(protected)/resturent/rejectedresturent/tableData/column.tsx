...
import { useApproveResturent, useDeleteResturent, useRejectResturent } from "@/hooks/useResturent"
...
export const columns: ColumnDef<Restaurant>[] = [
  ...
  },
  {
   ...
   const { mutate: deleteResturant, isPending: isDeletePending } = useDeleteResturent();
        ...
        const handleDeleteResturent = () => {
          deleteResturant(row.original._id, {
            onSuccess: () => {            
        ...
        <DropdownMenuContent align="end">
          <EditResturentModal restaurant={row.original} />
            {
                row.original.isApproved === "rejected" && 
               <>                               
                <DropdownMenuItem className="text-green-700" onClick={handleApprovedResturent} disabled={isPendingApprove}>  Approve{isPendingApprove && "ing..."}  </DropdownMenuItem>
                 <DropdownMenuItem className="text-red-700" onClick={handleDeleteResturent} disabled={isDeletePending}>  Delete{isDeletePending && "ing..."}  </DropdownMenuItem>
               </>
            }
